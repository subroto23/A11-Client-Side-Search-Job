...
const JobTypes = () => {
  ...
  return (
    <div className="border-2 rounder-md p-12">
      <div>
        <h1 className="md:text-2xl font-bold text-gray-600">Job Types</h1>
        <div className="flex justify-center items-center my-3">
          <hr className="w-20 border-2 border-green-500" />
          <hr className="w-full border border-gray-200" />
        </div>
        <div className="flex flex-col  my-6">
          {ItemsName("Freelance", 11, 1)}
          {ItemsName("Part Time", 5, 2)}
          {ItemsName("Full Time", 15, 3)}
          {ItemsName("Internship", 1, 4)}
        </div>
      </div>
    </div>
  );
